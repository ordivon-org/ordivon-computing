...
from anc_canonical import JsonValue, canonical_digest, validate_digest, validate_json_value
...
class HarnessToolStepIntent:
    ...
    def __post_init__(self) -> None:
        _text(self.intent_id, "Tool Step Intent identity", prefix="harness-tool-step-intent")
    ...
    def from_dict(cls, value: dict[str, Any]) -> HarnessToolStepIntent:
        ...
        if value["schemaVersion"] != 1 or value["kind"] != "ordivon.harness-tool-step-intent":
            raise HarnessProtocolError("HarnessToolStepIntent version or kind is invalid")
        try:
            consequence = HarnessRecoveryConsequence(value["recoveryConsequence"])
        except (TypeError, ValueError) as error:
            raise HarnessProtocolError("HarnessToolStepIntent consequence is invalid") from error
        return cls(
...
class HarnessToolStepReceipt:
    ...
    def __post_init__(self) -> None:
        _text(self.receipt_id, "Tool Step Receipt identity", prefix="harness-tool-step-receipt")
        ...
        if self.status is HarnessToolStepStatus.REJECTED and self.runtime_job_ref is not None:
        ...
        if self.status in {
            ...
            raise HarnessProtocolError("terminal Tool Step Receipt requires an observation digest")
        if self.status is HarnessToolStepStatus.CANCEL_REQUESTED and self.runtime_job_ref is None:
            raise HarnessProtocolError("cancel-requested Tool Step requires a Runtime Job")
    ...
    def from_dict(cls, value: dict[str, Any]) -> HarnessToolStepReceipt:
        ...
        if value["schemaVersion"] != 1 or value["kind"] != "ordivon.harness-tool-step-receipt":
            raise HarnessProtocolError("HarnessToolStepReceipt version or kind is invalid")
        try:
            status = HarnessToolStepStatus(value["status"])
        except (TypeError, ValueError) as error:
            raise HarnessProtocolError("HarnessToolStepReceipt status is invalid") from error
        return cls(
...
class HarnessRunSnapshot:
    ...
    def __post_init__(self) -> None:
        _text(self.snapshot_id, "Harness Run Snapshot identity", prefix="harness-run-snapshot")
        _text(self.harness_run_id, "Harness Run identity", prefix="harness-run")
        _text(self.assignment_id, "Assignment identity", prefix="assignment")
        _integer(self.assignment_generation, "Assignment generation", minimum=1)
        _digest(self.assignment_digest, "Assignment digest")
        ...
        _nullable_text(self.effective_model_id, "effective model identity", max_bytes=300)
    ...
    def to_dict(self) -> dict[str, JsonValue]:
        return {
            ...
            "activeToolStepIntentDigests": list(
                self.active_tool_step_intent_digests
            ),
    ...
    def from_dict(cls, value: dict[str, Any]) -> HarnessRunSnapshot:
        ...
        if value["schemaVersion"] != 1 or value["kind"] != "ordivon.harness-run-snapshot":
            raise HarnessProtocolError("HarnessRunSnapshot version or kind is invalid")
        if not isinstance(value["remainingBudget"], dict):
            raise HarnessProtocolError("HarnessRunSnapshot remainingBudget must be an object")
        try:
            pause_reason = HarnessRunPauseReason(value["pauseReason"])
        except (TypeError, ValueError) as error:
            raise HarnessProtocolError("HarnessRunSnapshot pause reason is invalid") from error
        return cls(
