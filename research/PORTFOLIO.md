...
| ID | Maturity | Priority | Owner | Active line | Observed revision | Blocked by | Next action | Next falsifier |
|---|---|---|---|---|---|---|---|---|
| ANC-HARNESS-002 | M4 | P0 | ordivon-host | — | 264d96cb2325 | — | Execute HHO-P0 before capability comparison: move new Run, Provider Call, Tool Step, Snapshot, Trace, recovery, and completion-proposal persistence from Host extensions into an independent Harness Journal/CAS; provide a caller-neutral Run Contract, standalone Runner, optional Host adapter, legacy Host-backed reader, and no-dual-write cutover. Then complete HHO-P1 automatic observation before Track R R3. | One-shot or mature Provider Harness paths match the same bounded workloads with equal correctness, recovery, portability, and lower permanent cost, while no bare-model use case requires a first-party Loop. |
...
| ID | Maturity | Priority | Owner | Active line | Observed revision | Blocked by | Next action | Next falsifier |
|---|---|---|---|---|---|---|---|---|
| ANC-COMPILER-002 | M1 | P1 | ordivon-computing | — | — | ANC-HARNESS-002, ANC-VERIFY-001 | Preserve the completed derivation and source audit. After the independent Harness baseline and repeated Trial envelope are frozen and one active line exits, run TCG-P0 shadow projection and TCG-P1 single-Actor Working Set against the strong transcript-compaction-retrieval baseline; do not build Child Runs, a graph store, Agent VM, Runtime Worker, or continual Harness before those gates pass. | A transcript-centered Harness with bounded compaction or retrieval, current Artifacts, and ordinary Host Task semantics matches or exceeds the Temporal Cognitive Graph on verified outcome, false completion, continuation, token cost, repeated reads, and operator review with fewer durable objects. |
...
| 2026-linear-loop-to-temporal-cognitive-graph | completed | first-principles, source-audited, and frontier-evidence derivation of a typed temporal cognitive state, Working Set, Run Actor, Child Run, and governed HarnessRevision research program | Consume through deferred ANC-COMPILER-002. Do not add a graph store, Agent VM, Runtime Worker, or third active line before the transcript-compaction-retrieval baseline and shadow-projection gate are admitted. |
