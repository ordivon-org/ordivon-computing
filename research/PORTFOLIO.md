...
| ID | Maturity | Priority | Owner | Active line | Observed revision | Blocked by | Next action | Next falsifier |
|---|---|---|---|---|---|---|---|---|
| ANC-MULTI-001 | M1 | P1 | ordivon-computing | — | — | — | P4 candidate: freeze one Goal with two independent Tasks, two separately produced Artifacts, and one deterministic Join. Compare against a single Agent plus independent verifier under equal task evidence and consequence authority. Admit no multi-Agent framework, scheduler, organization layer, or Agent VM unless this minimum workload proves a missing responsibility. | A single Agent plus independent verifier matches or exceeds two independent branches on accepted result, error discovery, cost, duplicate-Effect rate, responsibility clarity, and recovery; or ordinary Host Task/Artifact semantics express the branch/join without a new shared coordination primitive. |
| ANC-ADAPT-001 | M5 | P1 | ordivon-computing | — | — | — | P3 falsified the first natural text-procedure Skill candidate: repeated UNKNOWN recovery burden was real, but the unchanged compiled Skill lost 6/10 to 8/10 against explicit Tool-contract facts on the repaired live development set and no treatment survived holdout. Keep ANC-ADAPT at M5; do not create a Skill repository/runtime/registry. Next run the bounded ANC-MULTI-001 branch/join falsifier. | Multi-participant adaptation fails to beat a strong single-Agent plus independent-verifier baseline, creates duplicate Effects or ambiguous responsibility, or requires a new coordination layer where ordinary Host Task and Artifact semantics suffice. |
