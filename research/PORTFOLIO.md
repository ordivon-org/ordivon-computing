...
| ID | Maturity | Priority | Owner | Active line | Observed revision | Blocked by | Next action | Next falsifier |
|---|---|---|---|---|---|---|---|---|
| ANC-COMPILER-002 | M1 | P1 | ordivon-computing | — | — | ANC-HARNESS-002, ANC-VERIFY-001 | Preserve the completed derivation. After Level A, the minimum observation path, repeated sequential Trials, and a strong transcript-compaction-retrieval baseline pass, run TCG-P0 shadow projection and TCG-P1 single-Actor Working Set. Run Actor scheduling is conditional rather than a prerequisite; do not build Child Runs, Prime/RLM integration, graph storage, Runtime Worker, or continual Harness before the preceding gate proves need. | A transcript-centered Harness with bounded compaction or retrieval, current Artifacts, and ordinary Host Task semantics matches or exceeds the Temporal Cognitive Graph on verified outcome, false completion, continuation, token cost, repeated reads, and operator review with fewer durable objects. |
...
| 2026-linear-loop-to-temporal-cognitive-graph | completed | first-principles, source-audited, and frontier-evidence derivation of a typed temporal cognitive state, Working Set, Run Actor, Child Run, and governed HarnessRevision research program | Consume through deferred ANC-COMPILER-002. Complete Level A and the strong sequential baseline first; Run Actor, Child Runs, Prime/RLM Engine integration, Runtime Worker, graph storage, and continual Harness remain conditional on reproduced failures and prior gates. |
