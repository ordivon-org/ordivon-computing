...
| 2026-web-network-adversarial-foundations | completed | R0 terminology, causal-chain, evidence, Agent-amplification, and World/Security insertion baseline | Use as the default Web/network adversarial analysis grammar together with the completed R1 authority model. |
| 2026-web-identity-origin-delegation | completed | R1 origin, browser authority, ambient session, cross-origin request, OAuth delegation, workload identity, generated-Tool authority, and adaptive attack/defense baseline | Use as the authority-path model together with the completed R2 interpretation model. |
| 2026-web-semantic-differentials | completed | R2 HTTP framing and transitions, URL/encoding order, field metadata, content typing, cache equivalence, downstream interpreter, and Agent differential-discovery baseline | Use as the interpretation-path model together with the completed R3 network-evidence model. |
| 2026-network-truth-partial-failure | completed | R3 DNS/DNSSEC, BGP/RPKI, IP/NAT, TCP/UDP, TLS service identity, QUIC migration, proxy/VPN composition, timeout/retry ambiguity, and Agent path-adaptation baseline | Use as the network-claim and partial-failure model together with the completed R4 cross-layer case grammar. |
| 2026-cross-layer-attack-chains | completed | R4 official case reconstruction across cloud authority, Exchange, request smuggling, BGP/DNS/TLS, supply-chain identity, cache persistence, Agent hijacking, and duplicate Effects | Use as the real-chain and residual-closure baseline together with the completed R5 executable range. |
| 2026-minimal-adversarial-range-r5 | completed | R5 deterministic owned range with 176 Agent-hijacking, response-loss, and interpretation-differential Trials plus exact Host/Game contract observations | Use as the deterministic boundary baseline together with the completed R6 real-model transfer evidence. |
| 2026-real-model-adversarial-transfer-r6 | completed | R6 real DeepSeek V4 transfer with live Host/Harness/Runtime Canary consequences, structural Tool-metadata poisoning, causal ablation, and native recoverable-denial validation | Retain exact Tool schemas, Assignment-scoped ToolGrant, typed deterministic denial, independent verification, and explicit UNKNOWN. Continue only with one other Provider or mature Harness and exact per-Turn Tool-definition binding. |
| 2026-ai-capability-governance-and-private-power | completed | Completed G0-G7 capability-governance study: compact power and claim model, dated Provider and institutional snapshots, R6-backed intervention-layer evidence, and governance/dependency distinction | No standing work. Reopen only for a concrete Provider event, migration decision, regulatory deployment constraint, or reproduced architecture failure; revalidate only the affected snapshot. |
| 2026-agent-native-adversarial-systems | deferred | Round 1 evidence and deferred strategic Security research source | Consume the completed Round 1 method plus R5/R6 boundary evidence; resume only through a new held-out falsifier rather than a standing Security platform. |
