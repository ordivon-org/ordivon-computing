...
| ANC-HUMAN-001 | M3 | reference | ordivon-computing | — | — | — | Consume the reduced H0 model and human-AI capability distinction. Start H1 only with one bounded measurement study; do not build general collection or profile infrastructure. | A bounded H1 study shows that the reduced state-transition model omits a necessary distinction or does not change a real inference beyond mature domain methods. |
