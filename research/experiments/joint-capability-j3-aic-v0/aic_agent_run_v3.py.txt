...
def run_one(cases: dict[str, Any], scenario: dict[str, Any], treatment: str, model: str, replicate: int, secret: Path) -> dict[str, Any]:
    ...
    run_id = f"harness-run:cojc-j3-aic-v3:{scenario['scenarioId']}:{treatment}:{model}:r{replicate}:{now}"
    ...
    contract = HarnessRunContract(
        ...
        harness_implementation_id="ordivon-harness@cojc-j3-aic-v3",
        ...
        system_manifest_ref=bound_ref(f"system:{scenario['scenarioId']}:{treatment}:{model}:r{replicate}:v2", "system-manifest", {"experiment": "COJC-J3-AIC-V3", "treatment": treatment, "model": model, "replicate": replicate}),
...
def main() -> None:
    ...
    parser.add_argument("--seed", type=int, default=202608253)
    ...
    for index, (sid, treatment, model, rep) in enumerate(schedule, start=1):
        ...
        rows.append(row)
        campaign = {"schemaVersion": 1, "kind": "ordivon.computing.cojc-j3-aic-fresh-agent-campaign-v3", "experimentId": "COJC-J3-AIC-CURRENTNESS-V0-S1-V3", "casesDigest": canonical_digest(cases), "scheduleSeed": args.seed, "plannedTrials": len(schedule), "completedTrials": len(rows), "rows": rows, "summary": summarize(rows)}
