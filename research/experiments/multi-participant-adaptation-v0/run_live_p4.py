...
def invoke(settings:DeepSeekSettings,*,scenario:dict[str,Any],participant:str,run_id:str,assignment_id:str,sequence:int,messages:tuple[dict[str,str],...])->dict[str,Any]:
    ...
    context=canonical_digest({'messages':list(messages),'participantRef':participant,'scenarioId':scenario['scenarioId']})
    req=AgentTurnRequest(harness_run_id=run_id,turn_id=f'turn:{run_id.split(":",1)[-1]}:{sequence}',sequence=sequence,assignment_id=assignment_id,context_digest=context,tool_catalog_digest=NO_TOOLS,messages=messages,tools=(),remaining_budget={'modelCalls':1,'toolCalls':0,'totalTokens':16384,'wallTimeMs':90000})
    ...
    return {'source':value['source'],'summary':value['summary'],'modelEvidence':{'requestDigest':req.digest,'providerRequestDigest':adapter.provider_request_digest(req),'resultDigest':result.digest,'modelCallId':result.model_call_id,'modelId':result.model_id,'effectiveModelId':result.effective_model,'usage':result.usage,'totalTokens':usage_tokens(result.usage),'rawResponseDigest':result.raw_response_digest}}
...
def run_treatment(settings:DeepSeekSettings,s:dict[str,Any],hidden:list[dict[str,Any]],treatment:str)->dict[str,Any]:
    ...
    if treatment=='single-reflect':
        ...
        second=invoke(settings,scenario=s,participant=participant,run_id=run_id,assignment_id=assignment,sequence=2,messages=(system,{'role':'user','content':base},{'role':'assistant','content':json.dumps({'source':first['source'],'summary':first['summary']},ensure_ascii=False)},{'role':'user','content':feedback})); candidates.append(artifact(f'{sid}:single:revision',participant,second,evaluate_candidate(s,second['source'],hidden)))
    else:
        for label in ('a','b'):
    ...
    return {'scenarioId':sid,'treatment':treatment,'valid':True,'candidates':candidates,'join':join,'hostCoordination':host,'tokens':sum(c['modelEvidence']['totalTokens'] for c in candidates),'acceptedCandidateCount':sum(c['evaluation']['authoritative']['allPassed'] for c in candidates),'authoritativeCasesPassed':sum(c['evaluation']['authoritative']['passed'] for c in candidates),'authoritativeCasesTotal':sum(c['evaluation']['authoritative']['total'] for c in candidates)}
def aggregate(rows:list[dict[str,Any]],treatment:str)->dict[str,Any]:
    ...
    return {'scenarios':len(r),'invalidScenarios':invalid,'acceptedGoals':sum(x['join']['accepted'] for x in r),'acceptedCandidates':sum(x['acceptedCandidateCount'] for x in r),'authoritativeCasesPassed':sum(x['authoritativeCasesPassed'] for x in r),'authoritativeCasesTotal':sum(x['authoritativeCasesTotal'] for x in r),'totalTokens':sum(x['tokens'] for x in r),'branchEffectIntents':sum(x['hostCoordination']['branchEffectIntentCount'] for x in r),'responsibilityAmbiguities':sum(x['hostCoordination']['responsibilityAmbiguous'] for x in r),'recoveryFailures':sum(not x['hostCoordination']['partialApplyRecoveryPassed'] for x in r),'rejectedAdvanceFailures':sum(x['join']['accepted'] is False and x['hostCoordination']['rejectedAdvanceBlocked'] is not True for x in r)}
def main()->int:
    ...
    settings=DeepSeekSettings.from_secret_file(args.secret,max_output_tokens=3200,timeout_seconds=90.0); corpus=json.loads((HERE/'fixtures/corpus.json').read_text())['scenarios']; hidden=json.loads((HERE/'fixtures/evaluator-cases.json').read_text())['casesByScenario']; plan=json.loads((HERE/'plan-v1.json').read_text())
