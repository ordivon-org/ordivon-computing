...
P0 has landed the independent Harness store, standalone core, Host external-executor boundary, and cutover controls, but performance, recovery-gap, production state-root, and cutover receipts remain open. P1 contract and gateway work may proceed against fixtures; production exporters remain disabled until P0 closeout. Formal repeated Harness Trials remain blocked until P1 Core can reconstruct one complete deterministic Host–Harness–Runtime trajectory from owner-native references and freeze its Observation Selection. P1 reports evidence completeness, not Trial validity or Candidate quality; those belong to R3 and the later [`../experiment-loop-v0/`](../experiment-loop-v0/) plan.
