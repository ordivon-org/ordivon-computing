...
def merged_trial(treatment, replicate, cases, secret, secret_name):
    shuffled = list(cases)
    random.Random(f"ex2-family-v2:{replicate}:{treatment}").shuffle(shuffled)
    chunks = [shuffled[:12], shuffled[12:]]
    scored_rows = []
    usage_total = {
        "promptTokens": 0,
        "completionTokens": 0,
...
def main():
    ...
    ap.add_argument("--output", type=pathlib.Path, default=ROOT / "evidence" / "family-transfer-live-v2.json")
    args = ap.parse_args()
    trials = []
