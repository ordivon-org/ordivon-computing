...
for r in range(1,a.count+1):
    cp=subprocess.run([sys.executable,str(ROOT/'run_transfer.py'),'--arm',a.arm,'--replicate',str(r)],text=True,capture_output=True)
    if cp.returncode!=0:
...
(ROOT/f'transfer-{a.arm.lower()}.json').write_text(json.dumps(rows,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
