...
Status: the R0–R2 evidence contract, Observation Minimum Core, B3 cross-owner Selection, and B4 Formal Runner/fault-cell acceptance are complete. The historical B5 DeepSeek campaign is now **provider-blocked and frozen**: Flash and Pro produced zero valid complete Trials under the frozen Patch contract, and additional DeepSeek canaries are not authorized. B6 remains blocked. Current Harness/evaluation progression is owned by `research/portfolio.json`, which requires a separate real-model independent durable Harness-to-Runtime canary before any minimal comparative Trial resumes.
...
The subsequent B5 DeepSeek attempts are retained as diagnostics, not as a completed baseline. The current execution program records `B5=blocked_provider_capability`, zero valid complete Trials, and no authorization for further DeepSeek canaries or B6. The current Ready Frontier is a separate package-boundary and real-model independent durable Harness-to-Runtime canary; only after that evidence passes may the portfolio admit the smallest comparative Trial still needed for a Harness retain/shrink/delete decision. The historical comparison rule remains unchanged: development diagnosis needs three valid complete comparable Trials, while an architecture decision requires five to ten selection-eligible Trials per competitive configuration when cost permits.
