...
def docs_for(task: dict[str, Any], arm: str) -> tuple[str, int, list[str]]:
    arm_info = CONTRACT['owners'][task['owner']]
    revision = arm_info['current']
    parts: list[str] = []
    paths: list[str] = []
...
def trial(item: tuple[int, dict[str, Any], str, int, dict[str, str]]) -> dict[str, Any]:
    ...
    blind = 'bundle-' + hashlib.sha256(
        f'{task["id"]}:{arm}:{replicate}:fd6'.encode()
    ).hexdigest()[:10]
    answer_obj, subject_usage = call(
        ...
        SUBJECT_SCHEMA,
        'submit_answer',
        f'fd5control:{task["id"]}:{blind}:r{replicate}:subject',
    )
    ...
    score, judge_usage = call(
        ...
        judge_schema(task),
        'submit_score',
        f'fd5control:{task["id"]}:{blind}:r{replicate}:judge',
    )
    return {
        'taskId': task['id'],
        'owner': task['owner'],
        ...
        'secretSlot': sec['slot'],
        'sourceRevision': CONTRACT['owners'][task['owner']]['current'],
        'sourcePaths': paths,
...
def main() -> None:
    ...
    for task_id, values in summary['byTask'].items():
        ...
        if not current['majorityCorrect']:
            blockers.append({'taskId': task_id, 'reason': 'current-docs-not-majority-correct'})
        if current['criticalOverinference'] > 0:
            blockers.append({'taskId': task_id, 'reason': 'current-critical-overinference'})
        if current['unsupportedAuthorityClaims'] > 0:
            blockers.append({'taskId': task_id, 'reason': 'current-unsupported-authority-claim'})
    ...
    output = {
        'schemaVersion': 1,
        'kind': 'ordivon.feynman-documentation.fd5-conservative-control-evidence',
