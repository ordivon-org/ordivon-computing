...
def trial(item: tuple[int, dict[str, Any], str, int, dict[str, str]]) -> dict[str, Any]:
    ...
    answer_obj, subject_usage = call(
        ...
        SUBJECT_SCHEMA,
        'submit_answer',
        f'fd4rewrite:{task["id"]}:{blind}:r{replicate}:subject',
    )
    ...
    score, judge_usage = call(
        ...
        judge_schema(task),
        'submit_score',
        f'fd4rewrite:{task["id"]}:{blind}:r{replicate}:judge',
    )
    return {
        'taskId': task['id'],
        'owner': task['owner'],
...
def main() -> None:
    ...
    output = {
        'schemaVersion': 1,
        'kind': 'ordivon.feynman-documentation.second-wave-preflight-evidence',
