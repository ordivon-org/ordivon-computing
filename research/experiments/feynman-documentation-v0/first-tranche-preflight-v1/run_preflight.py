...
def trial(item: tuple[int, dict[str, Any], str, int, dict[str, str]]) -> dict[str, Any]:
    ...
    answer_obj, subject_usage = call(
        ...
        SUBJECT_SCHEMA,
        'submit_answer',
        f'fd3rewrite:{task["id"]}:{blind}:r{replicate}:subject',
    )
    ...
    score, judge_usage = call(
        ...
        judge_schema(task),
        'submit_score',
        f'fd3rewrite:{task["id"]}:{blind}:r{replicate}:judge',
    )
    return {
        'taskId': task['id'],
        'owner': task['owner'],
...
def main() -> None:
    ...
    summary['admissible'] = not blockers and not failures and len(rows) == 36
    output = {
        'schemaVersion': 1,
        'kind': 'ordivon.feynman-documentation.first-tranche-preflight-evidence',
