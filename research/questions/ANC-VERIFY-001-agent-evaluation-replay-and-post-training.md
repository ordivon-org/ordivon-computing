...
Current status, maturity, and next action are owned by [`research/portfolio.json`](../portfolio.json). The earlier deferral condition was discharged by three live Host/Harness experiments: Codex App Server H3, Hermes ACP H4, and bidirectional Harness replacement H5, which exposed stable repeated identities for Task, Assignment, Harness Run, model, Tool catalog, Runtime Job, Artifact, usage, stop, verification, and recovery evidence.
...
The accepted Observation Minimum Core remains a non-authoritative evaluation dependency: owner-native Host, Harness, and Runtime evidence is exported read-only into a rebuildable Gateway and Selection/query path. Current execution gates are intentionally not duplicated here; [`research/portfolio.json`](../portfolio.json) owns the Ready Frontier.

[`../experiments/experiment-loop-v0/`](../experiments/experiment-loop-v0/) remains downstream of stable valid repeated Trial groups. CEL-R4 does not activate unrestricted self-evolution or create a production experiment platform; promotion beyond Track R still requires repeated consumers and measurable net deletion elsewhere.
