...
- [`JOINT-CAPABILITY-PROGRAMME-20260825.md`](JOINT-CAPABILITY-PROGRAMME-20260825.md) — active Cross-Owner Joint Capability / Capability Ecology programme: preserves owner-native truth while testing additive composition, complementarity, higher-order irreducibility, emergence candidates and genuine threshold/phase-transition candidates through natural consumer pressure and subtraction; J0 is frozen in [`experiments/joint-capability-j0-v0/`](experiments/joint-capability-j0-v0/);
...
- [`experiments/book-representation-effect-v0/`](experiments/book-representation-effect-v0/) — consumer × operation × persistence representation-effect programme: the same Result map has model-variant-dependent immediate value, does not improve a fixed 2200-byte persistent trace, fails to improve the natural full-source Result audit, and loses to a smaller operation-relative evidence projection there; Human effects remain transport-gated and unclaimed;
