...
| [`host-harness-independent-persistence-and-observation.md`](host-harness-independent-persistence-and-observation.md) | accepted / implementation advanced | Independent Host/Harness durability is current and the former Host-backed Harness compatibility/cutover path is retired; Observation Minimum Core survives only as bounded non-authoritative inspect/export infrastructure, with continuous production collection still not admitted. |
